...
class Criterion(nn.Module):
    ...
    def custom_masked_total_loss(self, data_dict, *args, **kwargs):
        ...
        l1_loss = self._l1(depth_pred, depth_gt)

        init_loss = self._l1(init, depth_gt)
        depth_loss = l1_loss[mask].mean() +0.01 *init_loss[mask].mean() 

        mask_pred = data_dict['mask_pred']
        mask_gt = data_dict['depth_gt_mask']
        self._bceloss = nn.BCELoss()

        mask_loss = self._bceloss(mask_pred,mask_gt)
        total_loss = depth_loss + 0.1*mask_loss
        return total_loss
    ...
    def smooth_loss(self, data_dict, *args, **kwargs):
        ...
        pred_sn = get_surface_normal_from_depth(depth_pred, fx, fy, cx, cy, original_size = (original_w, original_h))
