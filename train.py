...
def train_one_epoch(epoch):
    ...
    with tqdm(train_dataloader) as pbar:
        for data_dict in pbar:
            ...
            depth,init,c,mask = model(data_dict['rgb'], data_dict['depth'])
            n, h, w = data_dict['depth'].shape
            data_dict['depth_pred'] = depth.view(n, h, w)
            data_dict['init'] = init.view(n,h,w)
            data_dict['mask_pred'] = mask.view(n,h,w)
...
def train(start_epoch):
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        test_loss, metrics_result = test_one_epoch(test_dataloader, epoch)

        tensorboard_log.add_scalar("train_mean_loss", train_loss, epoch)
        tensorboard_log.add_scalar("test_mean_loss", test_loss, epoch)
